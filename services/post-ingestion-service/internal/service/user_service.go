...
// entity inherits a shard instead -- see doc/sharding.md), then writes
...
// record; see doc/sharding.md "What still has to stay in sync" for the
...
func (s *UserService) ListUsers(ctx context.Context) ([]UserWithProfile, error) {
	...
	for _, u := range users {
		...
		if err != nil {
			...
			// the sync gap documented in doc/sharding.md -- degrade
